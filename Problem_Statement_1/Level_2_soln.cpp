...
void Normal(int R, int C , int G);
void ToriodalWrap(int R, int C , int G);
void PatternClassifier(int R, int C , int G);

int main()
{
    int Rows,Columns,Gens,Choice;
    cout<<"Enter the grid dimensions Rows and Columns respectively: \n";
    cin>>Rows>>Columns;

    cout<<"\nChoose the mode from below by entering their index:\n1.Toroidal Wrap\n2.Pattern Classifier\n3.Normal\n\n";
    cin>>Choice;
    switch(Choice)
    {
        case 1:
           ...
           cin>>Gens;
           ToriodalWrap(Rows, Columns, Gens);
        break;
        
        case 2:
        cout<<"Use default step limit? (y/n)\n";
        char response;
        cin>>response;
        if(response == 'y' || response == 'Y')
        {
            Gens = 10;
            PatternClassifier(Rows , Columns , Gens);
        }
        else
        {
            cout<<"Enter custom step limit: \n";
            cin>>Gens;
            cout<<endl;
            PatternClassifier(Rows , Columns , Gens);
        }
...
void PatternClassifier(int R, int C , int G)
{
    ...
    memcpy(GenData[0], Grid, sizeof(Grid));
    memcpy(Temp , Grid , sizeof(Grid));

    if(InitialPopulation == 0)
    {
        cout<<"Classification: Extinct\n";
        cout<<"Extinction Step: 0\n";
    ...
    for(int m=0 ; m<G ;m++)
    {
        ...
        GenPopulation[m] = TempPopulation;

        memcpy(GenData[m + 1], Temp1, sizeof(Temp1));
        memcpy(Temp , Temp1 , sizeof(Temp1));


        FinalPopulation = TempPopulation;
    ...
    {
        cout<<"Classification: Extinct\n";
        cout<<"Extinction Step: "<<ExtinctionStep<<endl;
        cout<<"Final Population: 0\n";
        return;
    }
    ...
    for(int i=1; i<=G && !found ; i++)
    {
        for(int j=0; j<i ; j++)
        {
            if(memcmp(GenData[i], GenData[j],
                      sizeof(GenData[0])) == 0)
            {
                int period = i-j;
                ...
                if(period != 1)
                {
                    ...
                    cout<<"First Repeat Step: "<<i
                        <<" (matches Step "<<j<<")"<<endl;
                    cout<<"Final Population: "
                        <<FinalPopulation<<endl;
    ...
    if(!found)
    {
        cout<<"Classification: Active\n";
        cout<<"Reason: No repeat or extinction detected within K = "
            <<G<<" steps\n";
        cout<<"Final Population: "
            <<FinalPopulation<<endl;
    }
